...
def process_car_rental_data(data_date):
    ...
    spark = SparkSession.builder \
        ...
        .config("spark.jars", "gs://public_pipe_data/jars/spark-snowflake_2.12-2.15.0-spark_3.2.jar,gs://public_pipe_data/jars/snowflake-jdbc-3.16.0.jar") \
        .getOrCreate()

    # Define GCS file path based on the date argument
    gcs_file_path = f"gs://public_pipe_data/Car-Rental-Data/car_rental_daily_data/car_rental_{data_date}.json"
    ...
    transformed_df = validated_df.withColumn(
        ...
        (col("rental_period.end_date").cast("date").cast("long") - col("rental_period.start_date").cast("date").cast("long")).cast("int")
    )
    ...
    snowflake_options = {
        "sfURL": "https://f.snowflakecomputing.com",
        "sfAccount": "fjm214",
        "sfUser": "MPATHAK",
        "sfPassword": "0",
...
# Example usage: process_car_rental_data("20240803")
